...
class ProductVariant(models.Model):
    product = models.ForeignKey(Product, related_name="variants", on_delete=models.PROTECT)
...
class ReplayAuditLog(models.Model):
    dead_letter_task = models.ForeignKey(DeadLetterTask, on_delete=models.PROTECT, related_name="replay_logs")
...
class BaseOrder(models.Model):
    ...
    abnormality_reason = models.CharField(max_length=255, blank=True, default="")
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
    updated_at = models.DateTimeField(auto_now=True, db_index=True)
...
class PlatformOrder(models.Model):
    order = models.ForeignKey(Order, on_delete=models.PROTECT, related_name="platform_payloads")
    platform = models.CharField(max_length=20, choices=PLATFORM_CHOICES, db_index=True)
...
class LogisticsShipment(models.Model):
    ...
    abnormal_reason = models.CharField(max_length=255, blank=True, default="")
    updated_at = models.DateTimeField(auto_now=True, db_index=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class LogisticsTrackingEvent(models.Model):
    ...
    shipment = models.ForeignKey(LogisticsShipment, on_delete=models.PROTECT, related_name="tracking_events")
...
class OrderRemark(models.Model):
    order = models.ForeignKey(Order, on_delete=models.PROTECT, related_name="remarks")
