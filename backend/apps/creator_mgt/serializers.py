...
class CreatorSerializer(serializers.ModelSerializer):
    # 防御性字段：真实数据库可能缺失某些关联数据，用 getattr 安全获取
    ...
    def get_tier(self, obj):
        val = getattr(obj, "tier", None)
        return val if val else ""
    def get_region(self, obj):
        val = getattr(obj, "region", None)
        return val if val else ""
    def get_email(self, obj):
        val = getattr(obj, "email", None)
        return val if val else ""
    def get_whatsapp(self, obj):
        val = getattr(obj, "whatsapp", None)
        return val if val else ""
    def get_timezone(self, obj):
        val = getattr(obj, "timezone", None)
        return val if val else "UTC"
