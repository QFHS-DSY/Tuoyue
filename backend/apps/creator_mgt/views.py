...
class CreatorViewSet(BaseModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if settings.DEMO_MODE:
            mock = [
                {"id": 1, "handle": "tiktok_fashion_queen", "region": "US", "tier": "A", "followers": 1200000, "engagement_rate": 4.5},
                {"id": 2, "handle": "tech_review_pro", "region": "JP", "tier": "B", "followers": 520000, "engagement_rate": 3.8},
                {"id": 3, "handle": "beauty_by_anna", "region": "KR", "tier": "A", "followers": 980000, "engagement_rate": 5.2},
                {"id": 4, "handle": "fitness_life", "region": "US", "tier": "C", "followers": 180000, "engagement_rate": 2.9},
                {"id": 5, "handle": "food_explorer", "region": "SG", "tier": "B", "followers": 350000, "engagement_rate": 4.1},
                {"id": 6, "handle": "pet_lover_official", "region": "AU", "tier": "A", "followers": 760000, "engagement_rate": 6.0},
            ]
            return success_response(data=mock)
        try:
...
class CreatorAIInsightViewSet(BaseModelViewSet):
    ...
    # 路由 /api/creators/{creator_id}/ai-insight/ 传的是 creator_id 不是 pk
    lookup_url_kwarg = "creator_id"

    def retrieve(self, request, *args, **kwargs):
        try:
            # 按 creator_id 查找 AI Insight，不存在时返回空而非抛 404
            insight = CreatorAIInsight.objects.filter(creator_id=kwargs.get("creator_id")).first()
            if insight:
                return success_response(data=CreatorAIInsightSerializer(insight).data)
...
class DashboardAggregateView(APIView):
    ...
    def get(self, request):
        if settings.DEMO_MODE:
            mock = [
                {"creator_id": 1, "handle": "tiktok_fashion_queen", "region": "US", "tier": "A", "follower_growth_curve": [8500, 9200, 10500, 11200], "heat_index": 92},
                {"creator_id": 2, "handle": "tech_review_pro", "region": "JP", "tier": "B", "follower_growth_curve": [3200, 3500, 3800, 4100], "heat_index": 78},
                {"creator_id": 3, "handle": "beauty_by_anna", "region": "KR", "tier": "A", "follower_growth_curve": [7500, 8100, 8800, 9600], "heat_index": 88},
            ]
            return success_response(data={"items": mock})
...
class CreatorTikTokAuthLoginView(APIView):
    ...
    def get(self, request):
        # 优先使用 query 参数，回退到 settings 中的 TIKTOK_REDIRECT_URI
        redirect_uri = request.query_params.get("redirect_uri", "").strip()
        if not redirect_uri:
            redirect_uri = getattr(settings, "TIKTOK_REDIRECT_URI", "").strip()
        if not redirect_uri:
            raise ValidationError("redirect_uri is required (set TIKTOK_REDIRECT_URI in .env or pass as query param)")
        scope = request.query_params.get("scope", getattr(settings, "TIKTOK_SCOPES", "user.info.basic"))
    ...
    def get(self, request):
        if settings.DEMO_MODE:
            return success_response(data={"items": [
                {"id": 1, "handle": "tiktok_fashion_queen", "region": "US", "tier": "A", "followers": 1200000},
                {"id": 2, "handle": "tech_review_pro", "region": "JP", "tier": "B", "followers": 520000},
                {"id": 3, "handle": "beauty_by_anna", "region": "KR", "tier": "A", "followers": 980000},
                {"id": 4, "handle": "fitness_life", "region": "US", "tier": "C", "followers": 180000},
                {"id": 5, "handle": "food_explorer", "region": "SG", "tier": "B", "followers": 350000},
            ]})
        try:
            ...
            return success_response(data={"items": [], "error": str(e)})
