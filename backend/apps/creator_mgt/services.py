...
def build_tiktok_authorize_url(redirect_uri: str, scope: str, state: str) -> str:
    # 优先使用 TIKTOK_CLIENT_KEY（用户 .env 配置），其次回退 TIKTOK_CREATOR_APP_KEY
    client_key = getattr(settings, "TIKTOK_CLIENT_KEY", "") or getattr(settings, "TIKTOK_CREATOR_APP_KEY", "")
    if not client_key:
        raise ValidationError("TikTok Client Key is not configured. Set TIKTOK_CLIENT_KEY in .env")
    if not redirect_uri:
        raise ValidationError("redirect_uri is required")

    auth_base_url = getattr(settings, "TIKTOK_AUTH_BASE_URL", "https://www.tiktok.com/v2/auth/authorize/").strip()
    params = {
...
def exchange_tiktok_code(code: str, redirect_uri: str) -> dict:
    client_key = getattr(settings, "TIKTOK_CLIENT_KEY", "") or getattr(settings, "TIKTOK_CREATOR_APP_KEY", "")
    client_secret = getattr(settings, "TIKTOK_CLIENT_SECRET", "") or getattr(settings, "TIKTOK_CREATOR_APP_SECRET", "")
