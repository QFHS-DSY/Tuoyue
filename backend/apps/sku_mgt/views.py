    ...
    def post(self, request):
        ...
        with transaction.atomic():
            for i in range(0, len(items), chunk_size):
                ...
                inserted = Sku.objects.bulk_create(objs, batch_size=chunk_size, ignore_conflicts=True)
                created += len(inserted)
                if len(inserted) < len(objs):
                    logger.warning(
                        "bulk_create ignored %d conflict(s) in chunk %d",
                        len(objs) - len(inserted), i // chunk_size,
                    )
